...
def gold_parse_completeness_daily():
    ...
    return (
        rankings_with_date
        .groupBy("raid_night_date", "role")
        .agg(
            F.count("*").alias("player_rows"),
            F.sum(F.when(F.col("rank_percent").isNull(), F.lit(1)).otherwise(F.lit(0))).alias("null_rank_rows"),
        )
        .withColumn(
            "null_rank_pct",
            F.when(F.col("player_rows") > 0, F.col("null_rank_rows") / F.col("player_rows"))
             .otherwise(F.lit(0.0)),
        )
