...
def _parsed_profiles():
    ...
    return raw.withColumn("_profile", F.from_json("profile_json", _PROFILE_SCHEMA)).withColumn(
        "_ingested_at_ts", F.to_timestamp("_ingested_at")
    )
...
def silver_raiderio_player_scores():
    ...
    return parsed.withColumn(
        "_season_row", F.explode_outer("_profile.mythic_plus_scores_by_season")
    ).select(
        F.col("player_name"),
        F.col("realm_slug"),
...
def silver_raiderio_player_runs():
    ...
    recent = parsed.withColumn(
        "_run", F.explode_outer("_profile.mythic_plus_recent_runs")
    ).withColumn("source", F.lit("recent"))
    best = parsed.withColumn("_run", F.explode_outer("_profile.mythic_plus_best_runs")).withColumn(
        "source", F.lit("best")
    )
    ...
    return (
        selected.withColumn("_rn", F.row_number().over(dedupe))
        .filter(F.col("_rn") == 1)
        .drop("_rn")
    )
