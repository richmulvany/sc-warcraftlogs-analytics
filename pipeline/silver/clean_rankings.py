...
def silver_player_rankings():
    ...
    latest_window = Window.partitionBy("report_code").orderBy(
        F.col("_ingested_at").desc_nulls_last()
    )
    raw_latest = (
        raw.withColumn("_rn", F.row_number().over(latest_window))
        .filter(F.col("_rn") == 1)
        .drop("_rn")
    )
    ...
    fights_dps = (
        raw_latest.withColumn("parsed", F.from_json(F.col("rankings_json"), _RANKINGS_SCHEMA))
        .filter(F.col("parsed").isNotNull())
        .withColumn("fight_entry", F.explode("parsed.data"))
    )
    if has_hps_col:
        fights_hps = (
            raw_latest.filter(F.col("rankings_hps_json").isNotNull())
    ...
    def _role_df(fights_df, role_key: str, role_label: str):
        ...
        return (
            fights_df.filter(F.col(chars_col).isNotNull())
